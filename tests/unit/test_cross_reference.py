...
def test_proximity_fallback_links_image_to_preceding_text():
    entries = [
        {
            "uri": "rag://c/d#text-0",
            "type": "text",
    ...
    image = next(e for e in result if e["uri"].endswith("#image-0"))
    text = next(e for e in result if e["uri"].endswith("#text-0"))
    # proximity should produce a strong link (related), not just related_weak
    assert "rag://c/d#text-0" in image["related"]
    assert "rag://c/d#image-0" in text["related"]


def test_proximity_fallback_links_table_to_preceding_text():
    entries = [
        {
            "uri": "rag://c/d#text-0",
            "type": "text",
...
def test_proximity_does_not_override_caption_match():
    """Caption match (strong link) is not replaced by proximity."""
    entries = [
        {
            "uri": "rag://c/d#text-0",
            "type": "text",
            "text": "see 图1-1 for details",
            "heading_path": "Ch 1",
            "element_id": "el-0",
            "related": [],
        },
    ...
    result = build_cross_references(entries)
    image = next(e for e in result if e["uri"].endswith("#image-0"))
    # Should still have the caption-matched strong link
    assert "rag://c/d#text-0" in image["related"]
