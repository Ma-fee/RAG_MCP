...
class RebuildIndexService:
    def rebuild_keyword_index(
        ...
        *,
        ...
        try:
            result = self._build_and_persist_keyword_store(
            ...
            IndexPersistenceService(data_dir=data_dir, index_dir=final_index_dir).write_active_manifest(
                manifest
            )
        except Exception:
        ...
        if old_manifest and old_manifest.get("index_dir") != str(final_index_dir):
            ...
            IndexPersistenceService(data_dir=data_dir, index_dir=final_index_dir).remove_index_dir(
                old_dir
            )
    ...
    def _build_and_persist_keyword_store(
        ...
        *,
        ...
        for doc in documents:
            ...
            chunks = self._build_chunks_for_document(
                document=doc,
                source_dir=source_dir,
                min_chunk_length=min_chunk_length,
            )
        ...
        persistence = IndexPersistenceService(data_dir=data_dir, index_dir=index_dir)
        persistence.write_resource_store(entries=all_resource_entries)
        persistence.write_keyword_store(corpus_id=corpus_id, entries=entries)
    ...
    def _build_chunks_for_document(
        ...
        *,
        ...
        if document.file_type == "pdf":
            pdf_path = source_dir / document.relative_path
            if not pdf_path.exists():
                raise RuntimeError(f"TOC chunking failed for PDF {document.relative_path}") from FileNotFoundError(
            ...
            try:
                ...
                # TOC extraction failures should not block indexing; fallback chunking keeps pipeline available.
                pass
